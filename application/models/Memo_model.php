...
class Memo_model extends CI_Model
{
	...
	public function total_memokedatangan()
	{
		...
		$result = $this->db->get('mdwms_memo');
        return $result->row();
	}

	public function total_logbookqc()
	{
		$this->db->select('SUM(bitActive) as totallogbookqc');
		$this->db->group_by('bitActive');
		$this->db->order_by('totallogbookqc','desc');
	...
	{
		$this->db->select('SUM(bitOpen) as openmemo');
		$this->db->group_by('bitOpen');
		$this->db->order_by('openmemo','desc');
		$result = $this->db->get('mdwms_memo');
        return $result->row();
	}
	...
	{
		$this->db->select('SUM(bitOpen) as openlogbookqc');
		$this->db->group_by('bitOpen');
		$this->db->order_by('openlogbookqc','desc');
		$result = $this->db->get('mdwms_logbookqc');
        return $result->row();
	}
	...
	{
		$this->db->select('SUM(bitProgress) as progresslogbookqc');
		$this->db->group_by('bitProgress');
		$this->db->order_by('progresslogbookqc','desc');
		$result = $this->db->get('mdwms_logbookqc');
        return $result->row();
	}
	...
	{
		$this->db->select('SUM(bitHold) as holdlogbookqc');
		$this->db->group_by('bitHold');
		$this->db->order_by('holdlogbookqc','desc');
		$result = $this->db->get('mdwms_logbookqc');
        return $result->row();
	}
	...
	{
		$this->db->select('SUM(bitClose) as closememo');
		$this->db->group_by('bitClose');
		$this->db->order_by('closememo','desc');
		$result = $this->db->get('mdwms_memo');
        return $result->row();
	}
	...
	{
		return $this->db->get('trdwms_unloadingdetail')->result();
	}
	...
	{
		$this->db->insert_batch('mdwms_memo', $data);
	}
